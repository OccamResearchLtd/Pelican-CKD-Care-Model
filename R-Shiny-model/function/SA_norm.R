...
SA_norm <- function(vec.mean_se = NULL, 
                    vec.mean_l_u = NULL,
                    num.mean = NULL,
                    num.se = NULL,
                    num.lower = NULL,
  ...
  # calculate the two parameters for normal distribution
  se <- ifelse(is.null(se), (ub - lb) / (2 * qnorm(0.975)), se)
  
  output[2*n+1] <- ifelse(is.null(lb), mean - se * qnorm(0.975), lb)
  output[2*n+2] <- ifelse(is.null(ub), mean + se * qnorm(0.975), ub)
  
  # draw a random number from 0 to 1
  rand <- runif(1)
  # use "while" loop to avoid rand == 0 or 1, which is theoretically possible with 0 probability
  while(rand == 1 | rand == 0) {
    rand <- runif(1)
  }
  output[2] <- qnorm(rand, mean = mean, sd = se)
