...
SA_lognorm <- function(vec.mean_se = NULL, 
                       vec.mean_l_u = NULL,
                       num.mean = NULL,
                       num.se = NULL,
                       num.lower = NULL,
  ...
  # calculate the two parameters for lognormal distribution
  meanlog <- log(mean) 
  ublog <- ifelse(is.null(ub), NA, log(ub))
  lblog <- ifelse(is.null(lb), NA, log(lb))
  selog <- ifelse(is.null(se), (ublog - lblog) / (2 * qnorm(0.975)), sqrt(log((se/mean)^2+1))) 
  ### Warning: mixed up about mean and median of lognormal distribution at this step
  ### It works fine when selog is small
  
  output[2*n+1] <- ifelse(is.null(lb), exp(qnorm(0.025, mean = meanlog, sd = selog)), lb)
  output[2*n+2] <- ifelse(is.null(ub), exp(qnorm(0.975, mean = meanlog, sd = selog)), ub)
  ...
  output[2] <- exp(qnorm(rand, mean = meanlog, sd = selog))
