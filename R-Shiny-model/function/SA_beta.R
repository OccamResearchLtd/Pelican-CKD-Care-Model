...
SA_beta <- function(vec.mean_se = NULL, 
                    vec.mean_l_u = NULL,
                    num.mean = NULL,
                    num.se = NULL,
                    num.lower = NULL,
  ...
  # calculate the two parameters for beta distribution
  se <- ifelse(is.null(se), (ub - lb) / (2 * qnorm(0.975)), se)
  alpha <- ifelse(mean == 0, 0, mean * (mean * (1 - mean) / (se ^ 2) - 1))
  beta <- ifelse(mean == 0, 0, alpha * (1 / mean - 1))
  
  output[2*n+1] <- ifelse(is.null(lb), qbeta(0.025, shape1 = alpha, shape2 = beta), lb)
  output[2*n+2] <- ifelse(is.null(ub), qbeta(0.975, shape1 = alpha, shape2 = beta), ub)
  
  # draw a random number from 0 to 1
  rand <- runif(1)
  output[2] <- qbeta(rand, shape1 = alpha, shape2 = beta)
